				   ...
				   int img_dbl, int descr_width, int descr_hist_bins )
{
	...
	/*{
		struct feature* f0;
		f0 = (feature*)calloc( features->total, sizeof(struct feature) );
		cvCvtSeqToArray( features, &f0, CV_WHOLE_SEQ );
		export_features( "siftFeat.key", f0, features->total );
	}*/
...
static IplImage* create_init_img( IplImage* img, int img_dbl, double sigma )
{
	...
	//cv::Mat k = cv::getGaussianKernel(5,1.248999599679680, CV_64F);
	/*gray = cvCreateImage( cvSize( img->width, img->height ), IPL_DEPTH_8U, 1 );
	dbl = cvCreateImage( cvSize( img->width*2, img->height*2 ), IPL_DEPTH_8U, 1 );
...
static void test123(const void* srcarr, void* dstarr, int smooth_type,
          int param1, int param2, double sigma1, double sigma2)
{
	cv::Mat src;
	cv::Mat dst;
	cv::Mat dst0;
	...
	src = cv::cvarrToMat(srcarr);
	dst0 = cv::cvarrToMat(dstarr);
	dst = dst0;
	depth = CV_MAT_DEPTH(src.type());
	...
	*/
	...
	for( o = 0; o < octvs; o++ )
		for( i = 0; i < intvls + 3; i++ )
		{
			...
			/*sprintf(fn, "octave%d-scale%d.dat", o,i);
			cvSave(&fn[0], gauss_pyr[o][i]);*/

			/*gray8 = cvCreateImage( cvGetSize(gauss_pyr[o][i]), IPL_DEPTH_8U, 1 );
			cvConvertScale( gauss_pyr[o][i], gray8, 255.0, 0 );
			sprintf(fn, "octave%d-scale%d.pgm", o,i);
			cvSaveImage(&fn[0], gray8);			
			cvReleaseImage( &gray8 );*/
		}
...
static void getMinMax(IplImage* image, double* vmin, double* vmax)
{
	*vmin = 100000000000000000;
	*vmax = -*vmin;

	float* im = (float*)image->imageData;
	for (int i=0; i<image->height*image->width; i++)
	{
		*vmin = std::min((double)*vmin, (double)*im);
		*vmax = std::max((double)*vmax, (double)*im);
		im++;
	}
}
...
static IplImage*** build_dog_pyr( IplImage*** gauss_pyr, int octvs, int intvls )
{
	...
	for( o = 0; o < octvs; o++ )
		for( i = 0; i < intvls + 2; i++ )
		{
			...
			/*gray8 = cvCreateImage( cvGetSize(dog_pyr[o][i]), IPL_DEPTH_8U, 1 );
			double vmin, vmax;
			getMinMax(dog_pyr[o][i], &vmin, &vmax);
			double scale = 255.0/(vmax-vmin);
			cvConvertScale( dog_pyr[o][i], gray8, scale, -vmin );
			sprintf(fn, "DoGoctave%d-scale%d.pgm", o,i);
			cvSaveImage(&fn[0], gray8);
			cvReleaseImage( &gray8 );

			sprintf(fn, "DoGoctave%d-scale%d.dat", o,i);
			cvSave(&fn[0], dog_pyr[o][i]);*/
		}
				...
				for(c = SIFT_IMG_BORDER; c < dog_pyr[o][0]->width-SIFT_IMG_BORDER; c++)
				{
					...
					if( ABS( pixval32f( dog_pyr[o][i], r, c ) ) > prelim_contr_thr )
						if( is_extremum( dog_pyr, o, i, r, c ) )
						{
							...
							if( feat )
							{
								if (feat->x/2>20 && feat->x/2<21)
								{
									int x =1;
									feat->img_pt.x = feat->img_pt.x;
								}
...
static void calc_feature_oris( CvSeq* features, IplImage*** gauss_pyr )
{
	...
	for( i = 0; i < n; i++ )
	{
		...
		if (feat->scl > 2.0 && feat->scl<2.2)   //, feat.ori)
		{
			int i =0;
		}
...
static void print_hist( double*** hist, int d, int n )
{
